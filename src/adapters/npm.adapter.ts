    ...
    try {
      ...
      const packageJson: any = {
        ...
        files: customizations.typescript
          ? ['dist']
          : ['src', `!src/**/*.test.${ext}`, `!src/**/*.spec.${ext}`, '!src/**/__tests__/**'],
        scripts: {},
        keywords: [],
        author: '',
        license: 'MIT',
...
`;
      ...
      if (customizations.eslint) {
        devDependencies.push(...getEslintPackages({ typescript: customizations.typescript }));
      }
...
`;
            await writeFile(join(projectPath, 'src', '__tests__', testFile), testContent);
